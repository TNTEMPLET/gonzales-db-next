...
export default async function TeamsPage({
  ...
  if (!adminUser) {
    redirect(`/admin/login?next=/admin/teams?org=${currentOrg}`);
  }
      ...
      <section className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="mb-8">
          <AdminSectionHeader
            ...
            currentPath={`/admin/teams?org=${currentOrg}`}
            allowRolePreview={hasAdminRoleAtLeast(role, "ADMIN")}
            allowViewByUser={adminUser.isMaster}
          />
          <h1 className="mb-3 text-4xl font-bold tracking-tight md:text-5xl">Teams & Rosters</h1>
          <p className="max-w-3xl text-zinc-400">
            Manage team rosters, coach assignments, and player imports.
          </p>
        </div>
