...
export default async function DugoutPage({
  ...
  if (!adminUser) {
    redirect(`/admin/login?next=/admin/dugout?org=${currentOrg}`);
  }
      ...
      <section className="mx-auto max-w-6xl px-4 sm:px-6">
        <div className="mb-8">
          <AdminSectionHeader
            ...
            currentPath={`/admin/dugout?org=${currentOrg}`}
            allowRolePreview={hasAdminRoleAtLeast(role, "ADMIN")}
            allowViewByUser={adminUser.isMaster}
          />
          <h1 className="mb-3 text-4xl font-bold tracking-tight md:text-5xl">Dugout Feed</h1>
          <p className="max-w-3xl text-zinc-400">
            Moderate coach posts, safety discussions, and community feed interactions.
          </p>
        </div>
