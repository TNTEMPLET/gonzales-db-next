...
export function parseLeagueFilter(value: string | null): LeagueFilter {
  if (value === "littleleague" || value === "diamond") return value;
  return "all";
}

export function normalizeAgeGroup(value: string | undefined): string {
...
export function matchesLeagueFilter(ageGroup: string, filter: LeagueFilter): boolean {
...
function extractAssignments(game: Game): Assignment[] {
  ...
  for (const entry of source) {
    ...
    if (!officialId || !fullName || fullName.toLowerCase() === "unknown umpire") {
      continue;
    }
  ...
  const rows = games.map((game) => {
    ...
    return {
      ...
      umpires: assignments.map((a, i) => ({
        name: a.name,
        pay: assignmentPays[i] ?? assignmentPays[assignmentPays.length - 1] ?? 0,
      })),
...
export function buildUmpireReportRows(games: Game[]): UmpireReportRow[] {
  ...
  for (const game of games) {
    ...
    for (const [index, assignment] of assignments.entries()) {
      const assignmentPay = assignmentPays[index] ?? assignmentPays[assignmentPays.length - 1] ?? 0;
