...
describe("classicDoubleElimDiagram", () => {
  ...
  it("maps locked five-team brackets structurally without feeder-label heuristics", () => {
    const map = new Map<string, LayoutMatch>([
      ["1", match("1", "9U Westbank", "9U NORD")],
      ["2", match("2", "9U St. Charles", "9U Eastbank")],
      ["3", match("3", "9U Westbank", "9U Ascension")],
    ...
    const slots = resolveClassicDoubleElimSlots(map, { locked: true });
    assert.ok(slots);
    assert.equal(slots!.grandFinal?.officialGameNumber, "8");
  });
