...
describe("sendSmsViaTwilio", () => {
  it("posts SMS messages to Twilio with basic auth", async () => {
    ...
    try {
      ...
      assert.ok(captured?.url.includes("/Accounts/AC123/Messages.json"));
      assert.equal(captured?.init.method, "POST");
      assert.equal((captured?.init.headers as Record<string, string>).Authorization, `Basic ${Buffer.from("AC123:secret").toString("base64")}`);
