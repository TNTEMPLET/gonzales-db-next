...
async function fetchScoreboardEvents(widgetId: string, dayStart: string): Promise<ScoreboardEvent[]> {
...
export async function findCreatedScoreboardEvent(
  widgetId: string,
  request: CreateGameRequest,
  scheduledForIso?: string,
): Promise<ScoreboardEvent | undefined> {
  ...
  for (let attempt = 0; attempt < 6; attempt += 1) {
    const events = await fetchScoreboardEvents(widgetId, dayStart);
...
export async function findCreatedEventId(
  ...
  const event = await findCreatedScoreboardEvent(widgetId, request, scheduledForIso);
  return event?.id;
}
