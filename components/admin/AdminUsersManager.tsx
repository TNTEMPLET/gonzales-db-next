...
export default function AdminUsersManager({
  ...
  const selectedUsers = useRowSelection<string>();
  ...
  return (
    <section className="space-y-6">
      ...
      <div className="grid grid-cols-1 gap-6">
        <div className="rounded-xl border border-zinc-800 bg-zinc-900/70 p-5 space-y-4">
          ...
          <BulkEmailToolbar
            ...
            disabled={busy}
            onSelectPage={() =>
              selectedUsers.selectMany(
                paginatedRegisteredUsers.filter((u) => !u.isBlocked).map((u) => u.id),
              )
            }
            onClear={selectedUsers.clear}
                ...
                >
                  <label className="flex items-start gap-2 flex-1 min-w-0 cursor-pointer">
                    <input
                      ...
                      checked={selectedUsers.selected.has(user.id)}
                      onChange={(event) => selectedUsers.toggle(user.id, event.target.checked)}
                    />
      ...
      <SendEmailModal
        ...
        targetOrg={targetOrg}
        isMasterAdmin={currentAdminIsMaster}
        registeredUserIds={Array.from(selectedUsers.selected)}
        onSent={(result) => {
          if (typeof result.sent === "number") {
            setNotice(`Email sent. Delivered ${result.sent}; failed ${result.failed ?? 0}.`);
          } else {
            setNotice(
              `Campaign created and submitted for approval (${result.recipients ?? selectedUsers.size} recipients). Open Communications to track it.`,
            );
          }
          selectedUsers.clear();
        }}
