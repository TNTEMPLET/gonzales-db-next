"use client";

import { useEffect, useState } from "react";

import BracketTeamPicker from "@/components/admin/BracketTeamPicker";
import type { BracketMatch, BracketRound, BracketSpec } from "@/lib/tournament-brackets/bracketSpec";
import {
  canAutoGenerateSingleEliminationRounds,
  generateSingleEliminationRoundsFromTeams,
  getSupportedSingleElimAutoSizes,
} from "@/lib/tournament-brackets/generateSingleElimFromTeams";
import {
  bracketFormatForChampionshipSeriesStyle,
  isDoubleEliminationFormat,
  resolveChampionshipSeriesStyle,
  type ChampionshipSeriesStyle,
} from "@/lib/tournament-brackets/bracketFormat";
import {
  canAutoGenerateDoubleEliminationRounds,
  generateDoubleEliminationRoundsForFormat,
} from "@/lib/tournament-brackets/generateDoubleElimFromTeams";
import {
  appliesDoubleElimClassicLayoutTemplate,
  CLASSIC_DOUBLE_ELIM_LAYOUT_LOCKED_ADMIN_MESSAGE,
  classicDoubleElimLayoutLockPatch,
  resolveDoubleElimClassicLayoutGenerationOptions,
} from "@/lib/tournament-brackets/doubleEliminationClassicLayoutTemplate";
import {
  buildRoundsFromOfficialTemplate,
  getOfficialTemplate,
  officialTemplateChampionshipLabel,
} from "@/lib/tournament-brackets/officialTemplates";

type Props = {
  spec: BracketSpec;
  projectId: string;
  projectUpdatedAt: string;
  organizationId: string;
  seasonYear: number;
  busy: boolean;
  /** When true (published READY or classic layout lock), structural edits are disabled. */
  structureLocked?: boolean;
  publishedLocked?: boolean;
  classicLayoutLocked?: boolean;
  onSave: (patch: {
    teams: string[];
    rounds: BracketRound[];
    bracketFormat: BracketSpec["bracketFormat"];
    championshipSeriesStyle?: ChampionshipSeriesStyle;
    rosterAgeGroup?: string | null;
  }) => Promise<void>;
};

function newId(prefix: string) {
  return `${prefix}-${typeof crypto !== "undefined" && crypto.randomUUID ? crypto.randomUUID() : String(Date.now())}`;
}

export default function BracketStructureEditor({
  spec,
  projectId,
  projectUpdatedAt,
  organizationId,
  seasonYear,
  busy,
  structureLocked = false,
  publishedLocked = false,
  classicLayoutLocked = false,
  onSave,
}: Props) {
  const [bracketFormat, setBracketFormat] = useState<BracketSpec["bracketFormat"]>(spec.bracketFormat);
  const [championshipSeriesStyle, setChampionshipSeriesStyle] = useState<ChampionshipSeriesStyle>(
    resolveChampionshipSeriesStyle(spec) ?? "winner_take_all",
  );
  const [teams, setTeams] = useState<string[]>([]);
  const [rosterAgeGroup, setRosterAgeGroup] = useState("");
  const [rounds, setRounds] = useState<BracketRound[]>([]);

  useEffect(() => {
    // eslint-disable-next-line react-hooks/set-state-in-effect -- Reset local editor drafts when a different project/spec is loaded.
    setBracketFormat(spec.bracketFormat);
    setChampionshipSeriesStyle(
      resolveChampionshipSeriesStyle({
        bracketFormat: spec.bracketFormat,
        championshipSeriesStyle: spec.championshipSeriesStyle,
      }) ?? "winner_take_all",
    );
    setTeams(spec.teams.filter(Boolean));
    setRosterAgeGroup(spec.rosterAgeGroup?.trim() ?? "");
    setRounds(
      spec.rounds.length > 0
        ? spec.rounds.map((r) => ({
            ...r,
            matches: r.matches.map((m) => ({ ...m })),
          }))
        : [],
    );
  }, [
    projectId,
    projectUpdatedAt,
    spec.bracketFormat,
    spec.championshipSeriesStyle,
    spec.rosterAgeGroup,
    spec.teams,
    spec.rounds,
  ]);

  async function handleSave() {
    const hadRounds = spec.rounds.some((r) => r.matches.length > 0);
    if (hadRounds && !rounds.some((r) => r.matches.length > 0)) {
      window.alert(
        "Cannot save: bracket rounds are empty in the editor. Reload the page and try again before saving schedule changes.",
      );
      return;
    }
    const patch: {
      teams: string[];
      rounds: BracketRound[];
      bracketFormat: BracketSpec["bracketFormat"];
      championshipSeriesStyle?: ChampionshipSeriesStyle;
      rosterAgeGroup?: string;
      classicDoubleElimLayoutLocked?: boolean;
    } = { teams, rounds, bracketFormat, ...classicDoubleElimLayoutLockPatch(teams, bracketFormat) };
    if (isDoubleEliminationFormat(bracketFormat)) {
      patch.championshipSeriesStyle = championshipSeriesStyle;
    }
    const roster = rosterAgeGroup.trim();
    if (roster) patch.rosterAgeGroup = roster;
    await onSave(patch);
  }

  function roundsWithoutIfNecessary(roundsToFilter: BracketRound[]): BracketRound[] {
    return roundsToFilter
      .map((round) => ({
        ...round,
        matches: round.matches.filter((match) => match.championshipRole !== "if_necessary"),
      }))
      .filter((round) => round.matches.length > 0);
  }

  function gameScheduleByNumber(
    sourceRounds: BracketRound[],
  ): Map<number, Pick<BracketMatch, "dateLabel" | "time" | "field">> {
    const byGame = new Map<number, Pick<BracketMatch, "dateLabel" | "time" | "field">>();
    for (const match of sourceRounds.flatMap((round) => round.matches)) {
      const gameNumber = Number(match.officialGameNumber);
      if (!Number.isInteger(gameNumber)) continue;
      byGame.set(gameNumber, {
        ...(match.dateLabel ? { dateLabel: match.dateLabel } : {}),
        ...(match.time ? { time: match.time } : {}),
        ...(match.field ? { field: match.field } : {}),
      });
    }
    return byGame;
  }

  function roundsForChampionshipSeriesStyle(style: ChampionshipSeriesStyle): BracketRound[] {
    if (style === "winner_take_all") {
      return roundsWithoutIfNecessary(rounds);
    }
    if (spec.officialTemplateId) {
      return buildRoundsFromOfficialTemplate(spec.officialTemplateId, teams, {
        championshipSeriesStyle: style,
        scheduleByGame: gameScheduleByNumber(rounds),
      });
    }
    return generateDoubleEliminationRoundsForFormat(teams, bracketFormatForChampionshipSeriesStyle(style), {
      ...resolveDoubleElimClassicLayoutGenerationOptions(teams, bracketFormatForChampionshipSeriesStyle(style)),
      championshipSeriesStyle: style,
    });
  }

  function updateChampionshipSeriesStyle(style: ChampionshipSeriesStyle) {
    try {
      const nextFormat = bracketFormatForChampionshipSeriesStyle(style);
      const nextRounds = roundsForChampionshipSeriesStyle(style);
      setChampionshipSeriesStyle(style);
      setBracketFormat(nextFormat);
      setRounds(nextRounds);
    } catch (err: unknown) {
      const message = err instanceof Error ? err.message : String(err);
      window.alert(`Could not change championship series: ${message}`);
    }
  }

  function handleGenerateRoundsFromTeams() {
    const canSingle = canAutoGenerateSingleEliminationRounds(teams, "single_elimination");
    const officialTemplate = spec.officialTemplateId ? getOfficialTemplate(spec.officialTemplateId) : undefined;
    const canDouble =
      isDoubleEliminationFormat(bracketFormat) &&
      (officialTemplate
        ? teams.map((team) => team.trim()).filter(Boolean).length === officialTemplate.teamCount
        : canAutoGenerateDoubleEliminationRounds(teams, bracketFormat));
    if (bracketFormat === "single_elimination" && !canSingle) return;
    if (isDoubleEliminationFormat(bracketFormat) && !canDouble) return;
    if (rounds.length > 0) {
      const label = isDoubleEliminationFormat(bracketFormat)
        ? bracketFormat === "modified_double_elimination"
          ? "modified double-elimination"
          : "double-elimination"
        : "single-elimination";
      const ok = window.confirm(
        `Replace all existing rounds with an auto-built ${label} bracket from your team list?`,
      );
      if (!ok) return;
    }
    if (isDoubleEliminationFormat(bracketFormat)) {
      setRounds(
        officialTemplate
          ? buildRoundsFromOfficialTemplate(officialTemplate.id, teams, {
              championshipSeriesStyle,
              scheduleByGame: gameScheduleByNumber(rounds),
            })
          : generateDoubleEliminationRoundsForFormat(teams, bracketFormat, {
            ...resolveDoubleElimClassicLayoutGenerationOptions(teams, bracketFormat),
            championshipSeriesStyle,
          }),
      );
    } else {
      setRounds(generateSingleEliminationRoundsFromTeams(teams));
    }
  }

  function addRound() {
    setRounds((prev) => [
      ...prev,
      {
        id: newId("round"),
        label: `Round ${prev.length + 1}`,
        matches: [{ id: newId("m"), home: "Home", away: "Away" }],
      },
    ]);
  }

  function removeRound(roundIndex: number) {
    setRounds((prev) => prev.filter((_, i) => i !== roundIndex));
  }

  function updateRoundLabel(roundIndex: number, label: string) {
    setRounds((prev) => prev.map((r, i) => (i === roundIndex ? { ...r, label } : r)));
  }

  function updateRoundBracketSection(
    roundIndex: number,
    section: BracketRound["bracketSection"] | "",
  ) {
    setRounds((prev) =>
      prev.map((r, i) => {
        if (i !== roundIndex) return r;
        if (!section) {
          const next = { ...r };
          delete next.bracketSection;
          return next;
        }
        return { ...r, bracketSection: section };
      }),
    );
  }

  function addMatch(roundIndex: number) {
    setRounds((prev) =>
      prev.map((r, i) =>
        i === roundIndex
          ? { ...r, matches: [...r.matches, { id: newId("m"), home: "Home", away: "Away" }] }
          : r,
      ),
    );
  }

  function removeMatch(roundIndex: number, matchIndex: number) {
    setRounds((prev) =>
      prev.map((r, i) =>
        i === roundIndex ? { ...r, matches: r.matches.filter((_: BracketMatch, j: number) => j !== matchIndex) } : r,
      ),
    );
  }

  function updateMatch(roundIndex: number, matchIndex: number, field: "home" | "away", value: string) {
    setRounds((prev) =>
      prev.map((r, ri) => {
        if (ri !== roundIndex) return r;
        return {
          ...r,
          matches: r.matches.map((m: BracketMatch, mi: number) =>
            mi === matchIndex ? { ...m, [field]: value } : m,
          ),
        };
      }),
    );
  }

  function updateOfficialGameNumber(roundIndex: number, matchIndex: number, value: string) {
    const trimmed = value.trim();
    setRounds((prev) =>
      prev.map((r, ri) => {
        if (ri !== roundIndex) return r;
        return {
          ...r,
          matches: r.matches.map((m: BracketMatch, mi: number) => {
            if (mi !== matchIndex) return m;
            if (!trimmed) {
              const next = { ...m };
              delete next.officialGameNumber;
              return next;
            }
            return { ...m, officialGameNumber: trimmed };
          }),
        };
      }),
    );
  }

  function updateMatchSchedule(
    roundIndex: number,
    matchIndex: number,
    field: "dateLabel" | "time" | "venue" | "field",
    value: string,
  ) {
    const trimmed = value.trim();
    setRounds((prev) =>
      prev.map((r, ri) => {
        if (ri !== roundIndex) return r;
        return {
          ...r,
          matches: r.matches.map((m: BracketMatch, mi: number) => {
            if (mi !== matchIndex) return m;
            if (!trimmed) {
              const next = { ...m };
              delete next[field];
              return next;
            }
            return { ...m, [field]: trimmed };
          }),
        };
      }),
    );
  }

  return (
    <div className="rounded-2xl border border-zinc-800 bg-zinc-900/40 p-4">
      <h2 className="text-sm font-semibold uppercase tracking-wide text-zinc-400">Bracket structure</h2>
      <p className="mt-1 text-xs text-zinc-500">
        {structureLocked ? (
          classicLayoutLocked ? (
            CLASSIC_DOUBLE_ELIM_LAYOUT_LOCKED_ADMIN_MESSAGE
          ) : (
            <>
              This bracket is published. Use <strong className="text-zinc-400">Team names</strong> above to rename teams
              without changing scores or layout. Schedule and game numbers below can still be updated.
            </>
          )
        ) : (
          <>
            Set bracket format and teams from the league roster for this age group, then build rounds automatically for
            single or double elimination (field sizes {getSupportedSingleElimAutoSizes().join(", ")} — non–power-of-two
            lists pad with trailing <strong className="text-zinc-400">BYE</strong> for higher seeds). You can still
            edit rounds by hand. Optional <strong className="text-zinc-400">official game #</strong> per match (from a
            published schedule) overrides automatic numbers in <strong className="text-zinc-400">G…</strong> row headers
            and <strong className="text-zinc-400">W…</strong> feeder labels when the bracket halves each round. Optional{" "}
            <strong className="text-zinc-400">date, time, field, park</strong> lines print on each game card.
          </>
        )}
      </p>

      <label className="mt-3 block text-xs font-medium text-zinc-400">
        Bracket format
        <select
          className="mt-1 w-full rounded-lg border border-zinc-700 bg-zinc-950 px-2 py-1.5 text-sm"
          value={bracketFormat}
          disabled={busy || structureLocked}
          onChange={(e) => {
            const format = e.target.value as BracketSpec["bracketFormat"];
            setBracketFormat(format);
            const style = resolveChampionshipSeriesStyle({ bracketFormat: format, championshipSeriesStyle: undefined });
            if (style) setChampionshipSeriesStyle(style);
          }}
        >
          <option value="unknown">Unknown</option>
          <option value="single_elimination">Single elimination</option>
          <option value="double_elimination">Double elimination</option>
          <option value="modified_double_elimination">Modified double elimination</option>
          <option value="pool_play">Pool play</option>
          <option value="custom">Custom</option>
        </select>
        {bracketFormat === "double_elimination" && appliesDoubleElimClassicLayoutTemplate(teams, bracketFormat) ? (
          <p className="mt-1 text-[11px] leading-relaxed text-zinc-500">
            Uses the classic unified layout template (G1–G9 diagram, champion column, if-necessary drop).
          </p>
        ) : null}
      </label>

      {isDoubleEliminationFormat(bracketFormat) ? (
        <label className="mt-3 block text-xs font-medium text-zinc-400">
          Championship series
          <select
            className="mt-1 w-full rounded-lg border border-zinc-700 bg-zinc-950 px-2 py-1.5 text-sm"
            value={championshipSeriesStyle}
            disabled={busy || publishedLocked}
            onChange={(e) => updateChampionshipSeriesStyle(e.target.value as ChampionshipSeriesStyle)}
          >
            <option value="winner_take_all">Modified — winner-take-all final (no if-necessary game)</option>
            <option value="always_scheduled_reset">Standard — grand final + if-necessary game</option>
          </select>
          <p className="mt-1 text-[11px] leading-relaxed text-zinc-500">
            {officialTemplateChampionshipLabel(championshipSeriesStyle)}
            {classicLayoutLocked && !publishedLocked
              ? " This can be changed on draft official brackets without unlocking the approved layout."
              : ""}
          </p>
        </label>
      ) : null}

      {!structureLocked ? (
        <BracketTeamPicker
          organizationId={organizationId}
          seasonYear={seasonYear}
          busy={busy}
          selectedTeamNames={teams}
          onSelectedTeamNamesChange={setTeams}
          rosterAgeGroup={rosterAgeGroup}
          championAgeGroupLabel={spec.championAgeGroupLabel}
          divisionLabel={spec.divisionLabel}
          onAgeGroupChange={setRosterAgeGroup}
        />
      ) : null}

      {(bracketFormat === "single_elimination" || isDoubleEliminationFormat(bracketFormat)) &&
      !structureLocked ? (
        <div className="mt-3">
          <button
            type="button"
            disabled={
              busy ||
              (bracketFormat === "single_elimination" &&
                !canAutoGenerateSingleEliminationRounds(teams, bracketFormat)) ||
              (isDoubleEliminationFormat(bracketFormat) &&
                !canAutoGenerateDoubleEliminationRounds(teams, bracketFormat))
            }
            className="rounded-lg border border-emerald-700/60 bg-emerald-950/40 px-3 py-2 text-xs font-semibold text-emerald-200 hover:bg-emerald-900/50 disabled:opacity-40"
            onClick={() => handleGenerateRoundsFromTeams()}
          >
            Build rounds from teams (
            {bracketFormat === "modified_double_elimination"
              ? "modified double"
              : isDoubleEliminationFormat(bracketFormat)
                ? "double"
                : "single"}{" "}
            elimination)
          </button>
        </div>
      ) : null}

      <div className="mt-4 space-y-4">
        {rounds.map((round, ri) => (
          <div key={round.id} className="rounded-lg border border-zinc-700 bg-zinc-950/50 p-3">
            <div className="flex flex-wrap items-center gap-2">
              <input
                className="min-w-0 flex-1 rounded border border-zinc-600 bg-zinc-950 px-2 py-1 text-sm"
                value={round.label}
                disabled={busy}
                onChange={(e) => updateRoundLabel(ri, e.target.value)}
              />
              {isDoubleEliminationFormat(bracketFormat) ? (
                <select
                  className="rounded border border-zinc-600 bg-zinc-950 px-2 py-1 text-xs text-zinc-300"
                  value={round.bracketSection ?? ""}
                  disabled={busy || structureLocked}
                  onChange={(e) =>
                    updateRoundBracketSection(
                      ri,
                      e.target.value as BracketRound["bracketSection"] | "",
                    )
                  }
                  aria-label={`Round ${ri + 1} bracket section`}
                >
                  <option value="">Section (auto)</option>
                  <option value="winners">Winners</option>
                  <option value="losers">Losers</option>
                  <option value="championship">Championship</option>
                </select>
              ) : null}
              <button
                type="button"
                disabled={busy || structureLocked}
                className="rounded border border-zinc-600 px-2 py-1 text-xs text-zinc-300 hover:bg-zinc-800"
                onClick={() => addMatch(ri)}
              >
                + Match
              </button>
              <button
                type="button"
                disabled={busy || structureLocked}
                className="rounded border border-red-900/50 px-2 py-1 text-xs text-red-300 hover:bg-red-950/40"
                onClick={() => removeRound(ri)}
              >
                Remove round
              </button>
            </div>
            <ul className="mt-2 space-y-2">
              {round.matches.map((m: BracketMatch, mi: number) => (
                <li key={m.id} className="flex flex-col gap-2 rounded border border-zinc-800/80 bg-zinc-950/40 p-2 text-sm">
                  <div className="flex flex-wrap items-center gap-2">
                    <input
                      className="min-w-[6rem] flex-1 rounded border border-zinc-600 bg-zinc-950 px-2 py-1 text-xs"
                      value={m.home}
                      disabled={busy || structureLocked}
                      onChange={(e) => updateMatch(ri, mi, "home", e.target.value)}
                      aria-label={`Round ${ri + 1} match ${mi + 1} home`}
                    />
                    <span className="text-zinc-500">vs</span>
                    <input
                      className="min-w-[6rem] flex-1 rounded border border-zinc-600 bg-zinc-950 px-2 py-1 text-xs"
                      value={m.away}
                      disabled={busy || structureLocked}
                      onChange={(e) => updateMatch(ri, mi, "away", e.target.value)}
                      aria-label={`Round ${ri + 1} match ${mi + 1} away`}
                    />
                    <label className="flex items-center gap-1 text-[11px] text-zinc-500">
                      <span className="shrink-0">Game #</span>
                      <input
                        className="w-14 rounded border border-zinc-600 bg-zinc-950 px-1.5 py-1 text-xs tabular-nums"
                        value={m.officialGameNumber ?? ""}
                        disabled={busy}
                        placeholder="—"
                        onChange={(e) => updateOfficialGameNumber(ri, mi, e.target.value)}
                        aria-label={`Round ${ri + 1} match ${mi + 1} official published game number`}
                      />
                    </label>
                    <button
                      type="button"
                      disabled={busy || structureLocked || round.matches.length < 2}
                      className="text-xs text-zinc-500 hover:text-red-300 disabled:opacity-30"
                      onClick={() => removeMatch(ri, mi)}
                    >
                      Remove
                    </button>
                  </div>
                  <div className="grid grid-cols-2 gap-2 sm:grid-cols-4">
                    <label className="text-[10px] text-zinc-500">
                      Date
                      <input
                        className="mt-0.5 w-full rounded border border-zinc-700 bg-zinc-950 px-1.5 py-1 text-[11px]"
                        value={m.dateLabel ?? ""}
                        disabled={busy}
                        placeholder="Sat 6/7"
                        onChange={(e) => updateMatchSchedule(ri, mi, "dateLabel", e.target.value)}
                      />
                    </label>
                    <label className="text-[10px] text-zinc-500">
                      Time
                      <input
                        className="mt-0.5 w-full rounded border border-zinc-700 bg-zinc-950 px-1.5 py-1 text-[11px]"
                        value={m.time ?? ""}
                        disabled={busy}
                        placeholder="6:00 PM"
                        maxLength={80}
                        onChange={(e) => updateMatchSchedule(ri, mi, "time", e.target.value)}
                      />
                    </label>
                    <label className="text-[10px] text-zinc-500">
                      Field
                      <input
                        className="mt-0.5 w-full rounded border border-zinc-700 bg-zinc-950 px-1.5 py-1 text-[11px]"
                        value={m.field ?? ""}
                        disabled={busy}
                        placeholder="Field 2"
                        onChange={(e) => updateMatchSchedule(ri, mi, "field", e.target.value)}
                      />
                    </label>
                    <label className="text-[10px] text-zinc-500">
                      Park / venue
                      <input
                        className="mt-0.5 w-full rounded border border-zinc-700 bg-zinc-950 px-1.5 py-1 text-[11px]"
                        value={m.venue ?? ""}
                        disabled={busy}
                        placeholder="Main complex"
                        onChange={(e) => updateMatchSchedule(ri, mi, "venue", e.target.value)}
                      />
                    </label>
                  </div>
                </li>
              ))}
            </ul>
          </div>
        ))}
      </div>

      <div className="mt-3 flex flex-wrap gap-2">
        {!structureLocked ? (
          <button
            type="button"
            disabled={busy}
            className="rounded-lg border border-zinc-600 px-3 py-2 text-xs font-semibold hover:bg-zinc-800"
            onClick={() => addRound()}
          >
            Add round
          </button>
        ) : null}
        <button
          type="button"
          disabled={busy}
          className="rounded-lg bg-red-700 px-3 py-2 text-xs font-semibold text-white hover:bg-red-600"
          onClick={() => void handleSave()}
        >
          {structureLocked ? "Save schedule & game numbers" : "Save bracket structure"}
        </button>
      </div>
    </div>
  );
}
